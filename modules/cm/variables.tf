...
variable "machine_type" {
  ...
  description = <<-EOD
  The machine type to use for BIG-IP Next CM VM; this may be a standard GCE
  machine type, or a customised VM ('custom-VCPUS-MEM_IN_MB'). Default value is
  'e2-standard-8'.
  EOD
}
...
variable "service_account" {
  ...
  description = <<-EOD
  The service account that will be used for the BIG-IP Next CM instances.
  EOD
}
...
variable "subnets" {
  ...
  validation {
    condition     = length(compact([for subnet in values(var.subnets) : can(regex("^https://www.googleapis.com/compute/v1/projects/[a-z][a-z0-9-]{4,28}[a-z0-9]/regions/[a-z][a-z-]+[0-9]/subnetworks/[a-z]([a-z0-9-]+[a-z0-9])?$", subnet)) ? "x" : ""])) == 1
    error_message = "Each subnets value must be a fully-qualified self-link URI."
  }
  description = <<-EOD
  Fully-qualified subnet self-links for each VPC network that BIG-IP Next CM
...
variable "bigip_next_service_account" {
  ...
  description = <<-EOD
  The service account used by BIG-IP Next instances. This will be used to create
  firewall rules to allow BIG-IP CM and BIG-IP Next instance communication.
  EOD
}
