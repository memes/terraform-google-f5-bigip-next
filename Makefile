...
EXAMPLES := ve-ea-standalone ve-ea-ha-cm
...
test/ephemeral/%/main.tf: $(wildcard examples/%/*.tf)
	...
	rsync -avP --exclude .terraform \
		...
		examples/$*/ $(@D)/
	sed -i '' -E -e '1h;2,$$H;$$!d;g' -e 's@module "bigip"[ \t]*{[ \t]*\n[ \t]*source[ \t]*=[ \t]*"[^"]+"@module "bigip" {\n  source = "../../../"@' $@
	sed -i '' -E -e '1h;2,$$H;$$!d;g' -e 's@module "cm"[ \t]*{[ \t]*\n[ \t]*source[ \t]*=[ \t]*"[^"]+"@module "cm" {\n  source = "../../../modules/cm/"@g' $@
...
tag.%:
	...
	@find examples -type f -name main.tf -print0 | \
		xargs -0 awk 'BEGIN{m=0;s=0}; /module "bigip"/ {m=1}; m==1 && /source[ \t]*=[ \t]*"(https:\/\/)?github.com\/memes\/terraform-google-f5-bigip-next\?ref=$*/ {s++}; END{if (s==0) { printf "%s has incorrect bigip source\n", FILENAME; exit 1}}'
	@find examples -type f -name main.tf -print0 | \
		xargs -0 awk 'BEGIN{m=0;s=0}; /module "cm"/ {m=1}; m==1 && /source[ \t]*=[ \t]*"(https:\/\/)?github.com\/memes\/terraform-google-f5-bigip-next\/\/modules\/cm\?ref=$*"/ {s++}; END{if (s==0) { printf "%s has incorrect cm source\n", FILENAME; exit 1 }}'
